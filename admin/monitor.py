...
def updateResult():
    ...
    result_list = []
    pr_list = ""
    if (len(prPair_list)>0):


        for prPair in prPair_list:
            pr_list += str(prPair[1]) + " " + str(prPair[2])  + " " + str(prPair[3])  + " " +str(prPair[4]) +" ; "
            result_list.append(str(prPair[0]))

        idlist_str = "(" + ', '.join(map(str, result_list)) + ")"
        markEmailedResultToDB(idlist_str)
        print("update " + str(len(idlist_str)) + "in database")
        notify_admin(str(len(prPair_list)),pr_list)
    else:
...
def notify_admin(num_newPRpairs,pr_list):
    cmd_str ="echo \" detect "+ num_newPRpairs +" duplicate PR pairs ===  "+ pr_list +" please check ASAP at http://128.2.112.25:5000/ :)\" | mail -s \"dupPR_bot: found new pairs\" shuruiz@andrew.cmu.edu"
    print(cmd_str)
    os.system(cmd_str)

def top_pair_similarityBiggerThanThreshold_unMarked(threshold):
    sql_str = "SELECT id, repo, pr1, pr2 , score\
...
def update_PR2Close_PR1Create_TimeInterval(repo, pr1, pr2):
    sql_select = "select `timestamp`, pr2_closed_at from duppr_pair_update where repo = %s and pr1 = %s and pr2 = %s"
    cur.execute(sql_select, (repo, pr1, pr2))
    rs = cur.fetchall()
    pr1_created_at, pr2_closed_at = rs[0][0], rs[0][1]
    conn.commit()  # save changes

    numDaysAfterPR2closed = (datetime.strptime(pr1_created_at, "%Y-%m-%dT%H:%M:%SZ")-datetime.strptime(pr2_closed_at, "%Y-%m-%dT%H:%M:%SZ")).days
    sql = "update duppr_pair_update set numDaysAfterPR2closed  = "+str(numDaysAfterPR2closed)+" where repo = %s and pr1 = %s and pr2 = %s"
    cur.execute(sql, (repo, pr1, pr2))
    conn.commit()
...
def updatePRstate():
    ...
    for line in data_dups:
        ...
        pr1_participant_list, pr1_num_comments, pr1_isMerged, pr1_closed_at = backend_interface.analyzePREvents(pr1_events)
        pr2_participant_list, pr2_num_comments, pr2_isMerged, pr2_closed_at = backend_interface.analyzePREvents(pr2_events)
        ...
        # if pr2 closed, check if pr1 is created after pr2 closed
        if pr2_status in ('closed', 'merged'):
            update_PR2Close_PR1Create_TimeInterval(repo, pr1, pr2)
